          ...
          <div>
            <ul>
              {languages[lang].months.map((month: string, i: number) => {
                return (
                  <li key={i}>
                    <MonthButton current={i === currentMonth} onClick={() => setMonth(i)} intensity={i} minNumberOfInterests={12}>
                      {month} <p style={{float: "right"}}>{i} interests</p>
                    </MonthButton>
                  </li>
                );
              })}
  ...
  function CalendarInner() {
    ...
    return (
      ...
      >
        ...
        <div id="gradient-container" style={{width: "100%", textAlign: "center",  justifyContent: "center", display: "flex"}}>
          <p>0 interest</p>
          <div id="gradient" style={{display: "inline-block", width: "70%", height: "10px", backgroundImage: "linear-gradient(to right, hsl(120, 100%, 50%), hsl(60, 100%, 50%), hsl(0, 100%, 50%))"}}></div>
          <p>{">=8"} interests</p>
        </div>
        <div>
          <div>
